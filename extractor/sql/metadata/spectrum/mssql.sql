...
FROM (
	...
	UNION ALL 
		...
		END	AS casting,
        CASE
            WHEN data_type in ('rowversion','varchar', 'char', 'varbinary', 'binary', 'nvarchar', 'nchar')AND CHARACTER_MAXIMUM_LENGTH <= 0  THEN 'varchar(65535)'
            WHEN data_type in ('rowversion','varchar', 'char', 'varbinary', 'binary', 'nvarchar', 'nchar')AND CHARACTER_MAXIMUM_LENGTH > 0  THEN 'varchar' + '(' + CAST(ABS(CHARACTER_MAXIMUM_LENGTH) * 2 AS VARCHAR) + ')'
        	WHEN data_type in ('numeric') THEN  'numeric' + '(' + CAST(ABS(NUMERIC_PRECISION)AS VARCHAR) + ',' + CAST(ABS(NUMERIC_SCALE)AS VARCHAR) + ')'
        	WHEN data_type in ('money', 'smallmoney') THEN  'numeric(15,4)'
        	...
        	WHEN data_type in ('double precision') CASE '${IS_SPECTRUM}' WHEN '1' THEN CASE '${HAS_ATHENA}' WHEN '1' THEN 'double' ELSE 'double precision' END ELSE 'double precision' END
            ...
            WHEN data_type in ('text','xml','xml2') THEN  'varchar(65535)'						
        	ELSE data_type
        END AS field_type ,
		'' AS json,
